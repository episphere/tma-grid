...
function sortRowsByRotatedPoints(rows, originAngle) {
  ...
  let sortingHelper = rows.map((row) => {
    const rotatedPoints = row.map((data) => rotatePoint(data["point"], -originAngle));
    const medianY = median(rotatedPoints.map((point) => point[1]));
    return {
      originalRow: row,
      medianY: medianY,
    };
  ...
  // Sort the rows based on the median Y value
  sortingHelper.sort((a, b) => a.medianY - b.medianY);
